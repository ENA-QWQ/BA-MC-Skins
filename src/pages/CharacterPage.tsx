import { useParams, Navigate } from 'react-router-dom';
import { useSkinData } from '../hooks/useSkinData';
import { useSiteConfig } from '../hooks/useSiteConfig';
import { SkinCard } from '../components/SkinCard';
import { Footer } from '../components/Footer';
import { Header } from '../components/Header';

function formatName(id: string): string {
    return id
        .split('-')
        .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' ');
}

export function CharacterPage() {
    const { gameName, characterName } = useParams<{ gameName: string; characterName: string }>();
    const { data, loading, error } = useSkinData();
    const { config } = useSiteConfig();

    if (loading) return <div className="loading-text">Loading...</div>;
    if (error) return <div className="error-text">Error: {error}</div>;

    const skins = data.filter(
        (s) => s.game === gameName && s.character === characterName
    );

    if (skins.length === 0) {
        return (
            <div className="not-found">
                <h2>Character not found</h2>
                <div className="back-link" onClick={() => window.history.back()}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M19 12H5M12 19l-7-7 7-7" />
                    </svg>
                    Back
                </div>
            </div>
        );
    }

    if (skins.length === 1) {
        return <Navigate to={`/skin/${skins[0].id}`} replace />;
    }

    const displayNameMap = config?.displayNameMap || {};
    const displayGameName = displayNameMap[gameName!] || formatName(gameName!);
    const displayCharacterName = formatName(characterName!);

    return (
        <div className="layout">
            <Header title={`${displayGameName} - ${displayCharacterName}`} showBack={true} />
            <section className="section">
                <div className="skins-grid">
                    {skins.map((skin) => (
                        <SkinCard key={skin.id} skin={skin} showUpdatedAt={true} showCharacterName={false} />
                    ))}
                </div>
            </section>
            <Footer />
        </div>
    );
}